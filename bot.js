...
client.on('ready', function(){
    var ms = 60000 ;
    var setGame = [`${client.guilds.size} Servers`,'Lhelp','Type Lhelp',`${client.users.size} Members`,'Linv','By: Fares | '];
    var i = -1;
    var j = 0;
...
『${prefix}invinfo <invitelink here> / لمعلومات عن الدعوه مثال : invinfo m82n8P
...
『Lsetwelcomer <text channel name> لاختيار روم للترحيب』
...
`]
    ...
    message.author.sendEmbed(embed).then(msg => {
        ...
        const backwards = msLcreateReactionCollector(backwardsFilter, { time: 2000000});
        const forwards = msLcreateReactionCollector(forwardsFilter, { time: 2000000});
...
client.on('message', function(msg) {
    if(msLcontent.startsWith (prefix  + 'server')) {
      let embed = new Discord.RichEmbed()
      .setColor('RANDOM')
      .setThumbnail(msLguild.iconURL)
      .setTitle(`**Showing Details Of** ${msLguild.name}`)
      .addField('`Server Region`',`[${msLguild.region}]`,true)
      .addField('`Roles Count`',`[${msLguild.roles.size}]`,true)
      .addField('`Members Count`',`[${msLguild.memberCount}]`,true)
      .addField('`Online Members`',`[${msLguild.members.filter(m=>m.presence.status == 'online').size}]`,true)
      .addField('`Text Channels`',`[${msLguild.channels.filter(m => m.type === 'text').size}]`,true)
      .addField('`Voice Channels`',`[${msLguild.channels.filter(m => m.type === 'voice').size}]`,true)
      .addField('`Server Owner`',`**${msLguild.owner}**`,true)
      .addField('`Server ID`',`**${msLguild.id}**`,true)
      .addField('`Server was created in`',msLguild.createdAt.toLocaleString())
      msLchannel.send({embed:embed})
    }
});
 client.on('message', message => {
...
client.on('message', msg => {
 if (msLcontent.startsWith(prefix + 'cal')) {
    let args = msLcontent.split(" ").slice(1);
        const question = args.join(' ');
    if (args.length < 1) {
        msLreply('Specify a equation, please.');
} else {    let answer;
    try {
    ...
    msLchannel.send(embed)  .catch(console.error);

    }
};
});
client.on('message', message => {
    if (message.content.startsWith("Ltrans")) {
    ...
    if (!language) {
        return message.reply(`Please supply valid agruments.\n**Example** \`Ltrans [text] to [language]\``);
    }
...
client.on('message', message => {
                if(message.content === prefix + "inv") {
                    let embed = new Discord.RichEmbed ()
                    embed.setTitle("**:arrow_right: Invite Lion Bot!**")
                    .setURL("https://discordapp.com/oauth2/authorize?client_id=344127240935571457&scope=bot&permissions=2146958591");
                   message.channel.sendEmbed(embed);
                  }
});
...
let reaction1 = msLcreateReactionCollector(reaction1Filter, { time: 12000 });
let reaction2 = msLcreateReactionCollector(reaction2Filter, { time: 12000 });
...
let EmbedBc = msLcreateReactionCollector(EmbedBcFilter, { time: 60000 });
let NormalBc = msLcreateReactionCollector(NormalBcFilter, { time: 60000 });
...
client.on('message', message => {
    ...
    if (command == "role") {
    if (!message.channel.guild) return;
    if(!message.guild.member(message.author).hasPermission("MANAGE_ROLES")) return message.reply("**:no_entry_sign:انت لا تملك صلاحيات **").then(msg => msLdelete(5000));;
    if(!message.guild.member(client.user).hasPermission("MANAGE_ROLES")) return message.reply("البوت لايملك صلاحيات ").then(msg => msLdelete(5000));;
    let user = message.mentions.users.first();
    if (message.mentions.users.size < 1) return message.reply('**ضع منشن الشخص!!**').then(msg => {msLdelete(5000)});
    let MRole = message.content.split(" ").slice(2).join(" ");
    if(!MRole)return message.reply("يجب عليك وضع اسم الرتبة").then(msg => {msLdelete(5000)});
    message.guild.member(user).addRole(message.guild.roles.find("name", MRole));
    message.reply('** Done ✅ **').then(msg => {msLdelete(10000)});
    }
    });
    client.on('message', message => {
    if (message.author.boss) return;
    ...
    message.channel.send(`الرجاء الانتظار حتى يتم الانتهاء من الامر`).then(msg => {
        ...
        msLedit(`تم الانتهاء من الامر ${message.guild.members.size}`);
    });
}
...
client.on('message', msg => {
  if (msLauthor.bot) return;
  if (!msLcontent.startsWith(prefix)) return;
  let command = msLcontent.split(" ")[0];
  command = command.slice(prefix.length);
  let args = msLcontent.split(" ").slice(1);
    ...
    if(msLmember.hasPermission("MANAGE_MESSAGES")) {
    if (textxt == "") {
        msLdelete().then
    msLchannel.send("***```ضع عدد الرسائل التي تريد مسحها 👌```***").then(m => m.delete(3000));
} else {
    msLdelete().then
    msLdelete().then
    msLchannel.bulkDelete(textxt);
...
if(!message.guild.member(client.user).hasPermission("MANAGE_ROLES")) return message.reply("**I Don't Have `MANAGE_ROLES` Permission**").then(msg => msLdelete(6000))
...
client.on('message', message => {
          let args = message.content.split(' ').slice(1);
   if(message.content.split(' ')[0] == 'Lcolor'){
...
.setColor("04791c")
 .setImage(`${item.type}`)
msLchannel.send(embed).then(() => {
        message.channel.awaitMessages(filter, { maxMatches: 1, time: 15000, errors: ['time'] })
        .then((collected) => {
         const sh = new Discord.RichEmbed()
.setColor("04791c")
.setDescription('**✅ |Good Job +1P**')
.addField('Type Lmypoints', 'To Show ur Points' , true)
.setFooter(message.author.username, message.author.avatarURL)
...
client.on('message', message => {
    if(message.content == 'Lvip-servers') {
    ...
    gimg = LiconURL;
    gmemb = Lmembers.size;
    gbots = Lmembers.filter(m=>m.bot).size;
    groles = Lroles.map(r=> {return r.name});
    let serv = new Discord.RichEmbed()
    .setAuthor(gname,gimg)
    .setThumbnail(gimg)
    .addField('Server Member Count',gmemb = Lmembers.size)
    .setColor('RANDOM')
    message.channel.send(`
    Server Name : **${gname}**
...
let reaction2 = msLcreateReactionCollector(reaction2Filter, { time: 12000 });
let reaction3 = msLcreateReactionCollector(reaction3Filter, { time: 12000 });
reaction1.on("collect", r => {
...
if (!points[message.author.id]) points[message.author.id] = {
  ...
  if(!message.guild) return;
    let id = message.author.id,prefix="L";
...
client.on('message', message => {
    if(!message.guild) return;
    if (message.author.bot) return;
      let id = message.author.id,prefix="L";//البريفكس
...
message.channel.sendEmbed(Embed).then(msg => {
    ...
    let blue = msLcreateReactionCollector(blueFilter, { time: 15000 });
    let orange = msLcreateReactionCollector(orangeFilter, { time: 15000 });
...
client.on('message', message => {
var cats = ["http://www.shuuf.com/shof/uploads/2015/09/09/jpg/shof_b9d73150f90a594.jpg","https://haltaalam.info/wp-content/uploads/2015/05/0.208.png","https://haltaalam.info/wp-content/uploads/2015/05/266.png","https://haltaalam.info/wp-content/uploads/2015/05/250.png","https://haltaalam.info/wp-content/uploads/2017/02/0.2517.png","https://pbs.twimLcom/media/CP0mi02UAAA3U2z.png","http://www.shuuf.com/shof/uploads/2015/08/31/jpg/shof_3b74fa7295ec445.jpg","http://www.shuuf.com/shof/uploads/2015/08/22/jpg/shof_fa3be6ab68fb415.jpg","https://pbs.twimLcom/media/CSWPvmRUcAAeZbt.png","https://pbs.twimLcom/media/B18VworIcAIMGsE.png"]
        var args = message.content.split(" ").slice(1);
...
client.on('ready', () => {
  ...
  client.guilds.forEach(g => {
    LfetchInvites().then(guildInvites => {
      invites[Lid] = guildInvites;
    });
  });
});
...
client.on('message', async msg => { // eslint-disable-line
	if (msLauthor.bot) return undefined;
	//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
	if (!msLcontent.startsWith(prefix)) return undefined;
	const args = msLcontent.split(' ');
	const searchString = args.slice(1).join(' ');
	//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
	const url = args[1] ? args[1].replace(/<(.+)>/g, '$1') : '';
	const serverQueue = queue.get(msLguild.id);
//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
	let command = msLcontent.toLowerCase().split(" ")[0];
	command = command.slice(prefix.length)
//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
	if (command === `play`) {
		const voiceChannel = msLmember.voiceChannel;
		if (!voiceChannel) return msLchannel.send('يجب توآجد حضرتك بروم صوتي .');
		const permissions = voiceChannel.permissionsFor(msLclient.user);
		if (!permissions.has('CONNECT')) {
			//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
			return msLchannel.send('لا يتوآجد لدي صلاحية للتكلم بهذآ الروم');
		}//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
		...
		if (!permissions.has('EMBED_LINKS')) {
			return msLchannel.sendMessage("**يجب توآفر برمشن `EMBED LINKS`لدي **")
		}
		...
		if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
			...
			return msLchannel.send(` **${playlist.title}** تم الإضآفة إلى قأئمة التشغيل`);
		} else {
...
//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
					.setFooter("Galaxy")
					msLchannel.sendEmbed(embed1).then(message =>{message.delete(20000)})

					// eslint-disable-next-line max-depth
					try {
						var response = await msLchannel.awaitMessages(msg2 => msg2.content > 0 && msg2.content < 11, {
							maxMatches: 1,
							time: 15000,
							errors: ['time']
						});//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
						...
						console.error(err);
						return msLchannel.send('لم يتم إختيآر مقطع صوتي');
					}
					const videoIndex = parseInt(response.first().content);
					var video = await youtube.getVideoByID(videos[videoIndex - 1].id);
				} catch (err) {
					console.error(err);
					return msLchannel.send(':X: لا يتوفر نتآئج بحث ');
				}
			}//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
		...
		if (!msLmember.voiceChannel) return msLchannel.send('أنت لست بروم صوتي .');
		if (!serverQueue) return msLchannel.send('لا يتوفر مقطع لتجآوزه');
		serverQueue.connection.dispatcher.end('تم تجآوز هذآ المقطع');
		return undefined;
	} else if (command === `stop`) {//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
		if (!msLmember.voiceChannel) return msLchannel.send('أنت لست بروم صوتي .');
		if (!serverQueue) return msLchannel.send('لا يتوفر مقطع لإيقآفه');
		serverQueue.songs = [];
		serverQueue.connection.dispatcher.end('تم إيقآف هذآ المقطع');
		return undefined;
	} else if (command === `vol`) {
		if (!msLmember.voiceChannel) return msLchannel.send('أنت لست بروم صوتي .');
		if (!serverQueue) return msLchannel.send('لا يوجد شيء شغآل.');
		if (!args[1]) return msLchannel.send(`:loud_sound: مستوى الصوت **${serverQueue.volume}**`);
		serverQueue.volume = args[1];//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
		serverQueue.connection.dispatcher.setVolumeLogarithmic(args[1] / 50);
		return msLchannel.send(`:speaker: تم تغير الصوت الي **${args[1]}**`);
	} else if (command === `np`) {
		if (!serverQueue) return msLchannel.send('لا يوجد شيء حالي ف العمل.');
		const embedNP = new Discord.RichEmbed()
	.setDescription(`:notes: الان يتم تشغيل : **${serverQueue.songs[0].title}**`)
		return msLchannel.sendEmbed(embedNP);
...
${serverQueue.songs.map(song => `**${++index} -** ${sonLtitle}`).join('\n')}
**الان يتم تشغيل** ${serverQueue.songs[0].title}`)
		return msLchannel.sendEmbed(embedqu);
	} else if (command === `pause`) {
		if (serverQueue && serverQueue.playing) {
			serverQueue.playing = false;
			serverQueue.connection.dispatcher.pause();
			return msLchannel.send('تم إيقاف الموسيقى مؤقتا!');
		}//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
		return msLchannel.send('لا يوجد شيء حالي ف العمل.');
	} else if (command === "resume") {
		if (serverQueue && !serverQueue.playing) {
			serverQueue.playing = true;
			serverQueue.connection.dispatcher.resume();
			return msLchannel.send('استأنفت الموسيقى بالنسبة لك !');
		}//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
		return msLchannel.send('لا يوجد شيء حالي في العمل.');
	}
...
//	console.log('yao: ' + Util.escapeMarkdown(video.thumbnailUrl));
	...
	if (!serverQueue) {
		...
		queue.set(msLguild.id, queueConstruct);
//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
...
//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
		try {
			...
			play(msLguild, queueConstruct.songs[0]);
		} catch (error) {
			console.error(`I could not join the voice channel: ${error}`);
			queue.delete(msLguild.id);
			return msLchannel.send(`لا أستطيع دخول هذآ الروم ${error}`);
		}
	} else {//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
		serverQueue.songs.push(song);
		console.log(serverQueue.songs);
		if (playlist) return undefined;
		else return msLchannel.send(` **${sonLtitle}** تم اضافه الاغنية الي القائمة!`);
	}
	return undefined;
}//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
...
function play(guild, song) {
	...
	console.log(serverQueue.songs);
//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
	const dispatcher = serverQueue.connection.playStream(ytdl(sonLurl))
	...
	serverQueue.textChannel.send(`بدء تشغيل : **${sonLtitle}**`);
}//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
...
${prefix}queue ⇏ لمعرفة قآئمة التشغيل
 ...
 client.on('message', message => {
            if(!message.channel.guild) return;
let args = message.content.split(' ').slice(1).join(' ');
if (message.content.startsWith('Lusers')){
...
client.on('message', msg => {
    if(msLcontent.startsWith(prefix + 'linkbot')) {
    if(msLchannel.type === 'dm') return;
const user = msLmentions.users.first();
if(!user) return msLchannel.send('**``' + 'قم بتحديد بوت' + '``**')
if(!user.bot) return msLreply('\`منشن بوت\`');
msLchannel.send(`**Bot InviteURL : ** https://discordapp.com/oauth2/authorize?client_id=${user.id}&scope=bot&permissions=2146958591`)
    }
});
...
client.on('message', message => {
    ...
    if(command == prefix + 'find') { // الامر : $find
        ...
        if(message.author.bot) return;
        if(!message.channel.guild) return;
        if(!args1) return message.channel.send(`**➥ Useage:** ${prefix}find (اي حرف من الاسم الي تبيه)`).then(msg => msLdelete(5000));
  ...
  if(message.content.startsWith(prefix + "giveaway")) {
    ...
    message.channel.send(`:eight_pointed_black_star:| **من فضلك اكتب اسم الروم**`).then(msgg => {
      message.channel.awaitMessages(filter, {
        ...
        collected.first().delete();
        msgLedit(':eight_pointed_black_star:| **اكتب مدة القيف اواي بالدقائق , مثال : 60**').then(msg => {
          message.channel.awaitMessages(filter, {
            max: 1,
            time: 20000,
            errors: ['time']
            ...
            collected.first().delete();
            msgLedit(':eight_pointed_black_star:| **واخيرا اكتب على ماذا تريد القيف اواي**').then(msg => {
              message.channel.awaitMessages(filter, {
                max: 1,
                time: 20000,
                errors: ['time']
                ...
                try {
                  ...
                  msgLedit(`:heavy_check_mark:| **تم اعداد القيف اواي**`);
                } catch(e) {
                  msgLedit(`:heavy_multiplication_x:| **لم اقدر على اعداد القيف اواي بسبب نقص الخصائص**`);
                  console.log(e);
                }
              });
            });
...
possibleInvites.push([inviteCount, exec]);
        ...
        .setColor('RANDOM')
        .addField(':دخولك لديسكورد', `${moment(heLcreatedTimestamp).format('YYYY/M/D HH:mm:ss')} **\n** \`${moment(heLcreatedTimestamp).fromNow()}\`` ,true)
...
client.on('message', message => {
if(message.content.includes("<@344127240935571457>")) {
...
client.on("message", (message) => {
    ...
    switch(command) {
        ...
        if(!message.guild.member(client.user).hasPermission("MANAGE_CHANNELS")) return message.reply("**I Don't Have `MANAGE_CHANNELS` Permission**").then(msg => msLdelete(6000))
        ...
        if (!message.channel.type =="text") return;
        if (!message.member.hasPermission("MANAGE_CHANNELS")) return message.reply("**Sorry, You Don't Have `MANAGE_CHANNELS` permission**")
        if(!message.guild.member(client.user).hasPermission("MANAGE_CHANNELS")) return message.reply("**I Don't Have `MANAGE_CHANNELS` Permission**").then(msg => msLdelete(10000))
        if (!message.mentions.members.first()) return message.reply("**Mention a user!🤔**")
